...
docker run --gpus all --network=host --env="DISPLAY" --env="QT_X11_NO_MITSHM=1"  --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" --name=contact_graspnet_final -t -d -v "$(dirname "$0")":/contact_graspnet -v /dev:/dev --device-cgroup-rule "c 81:* rmw"  --device-cgroup-rule "c 189:* rmw" contact_graspnet_final
